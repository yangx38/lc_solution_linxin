...
class Solution {
    public int oddEvenJumps(int[] A) {
        ...
        int numOfValidStartIndex = 1;

        for (int i = n - 2; i >= 0; --i) {
            Map.Entry ceil = map.ceilingEntry(A[i]);
            if (ceil != null) {
                odd[i] = even[(int)ceil.getValue()];
            }
            Map.Entry floor = map.floorEntry(A[i]);
            if (floor != null) {
                even[i] = odd[(int)floor.getValue()];
            }
            if (odd[i]) {
                numOfValidStartIndex++;
            }
            map.put(A[i], i);
        }
        return numOfValidStartIndex;
    }
}
// @lc code=end
